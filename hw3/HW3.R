...
SVGM <- function(df,bin=NULL,cut=1){
  ...
  z <- df[, 3]
  p <- gstat::variogram(z~1, loc = ~x + y, data=df, cloud=T, cutoff=max(d)/cut, 
                        width=bin, alpha=c(0, 90))
  plot(p, ylab=expression((Z(s[i]) - Z(s[j]))^2))
}
# Loading data --------------------
wipp.transmissivity <- read.table("E:/MATH532/HW3/WIPP_transmissivity.txt", 
                                  header=TRUE, quote="\"")
# Part 1 --------------------
#Plot the data, comment on any trends
ggplot(data=wipp.transmissivity, aes(x=x, y=y)) + 
  geom_point() +
  geom_text(data = wipp.transmissivity, aes(x, y, label = seq(1:length(x))),
            hjust = 2)
#Plot the semivariogram cloud for ALL pairs of points 
SVGM(wipp.transmissivity)

#How does the semivariogram cloud change when the large value of -10.12 is removed?
wipp <- wipp.transmissivity[-which(wipp.transmissivity$logT < -10), ]
SVGM(wipp)
